...
function paginate(items, q = {}) {
  ...
  if (q.sortBy) {
    out.sort((a, b) => {
      const av = a[q.sortBy], bv = b[q.sortBy];
      if (av === bv) return 0;
  ...
  return { data: out.slice((page - 1) * pageSize, page * pageSize), page, pageSize, total, totalPages: Math.max(1, Math.ceil(total / pageSize)) };
}
...
export const api = {
  ...
  auth: MOCK_MODE ? {
    ...
    resetPassword:  (token, password) => apiFetch(EP.auth.resetPassword, "POST", { token, password }),
